...
function buildQuery(data, locals) {
  ...
  if (locals && locals.site) {
    descriptors.siteSlugs = [locals.site.slug];
  }
  ...
  if (descriptors.pins) {
    const pinClauses = descriptors.pins
      .map(url => ({term: {canonicalUrl: url}}));

    elasticQuery.body.query.bool = {
      should: [
        {bool: elasticQuery.body.query.bool},
        ...pinClauses,
      ],
